...
import { ITypeAdditionRequest, ITypeAdditionResponse } from 'src/app/shared/interfaces/type-addition/type-addition.interfaces';
...
describe('AdminAdditionTypeComponent', () => {
  ...
  it('should edit an existing product', async () => {
    ...
    if (component.editStatus) {
      await additionProductService.updateFirebase(component.typeAdditionForm.value, '1').then(() => {  
        expect(mockToastService.showSuccess).toHaveBeenCalledWith('', 'Тип продукта змінено');
      })
    }
    expect(component.editStatus).toBeFalse();
  });
  ...
  it('should handle product deletion', async () => {
    ...
    expect(mockToastService.showSuccess).toHaveBeenCalledWith('', 'Тип продукта видалено');
  });
